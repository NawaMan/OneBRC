...
// Store bytes in a long array along with a hope to shorten the equals operations.
// Does not seems to help much.
...
public class CalculateAverage_NawaMan4 {
    ...
    static class StationName implements Comparable<StationName> {
        ...
        StationName(byte[] byteArray, long[] longArray, int byteLength, int longLength, int nameHash) {
...
//                return false;
            ...
            return Arrays.equals(longArray, 0, longLength, other.longArray, 0, longLength);
        }
        ...
        public String toString() {
            if (toString == null) {
                synchronized (this) {
                    if (toString == null) {
                        toString = new String(byteArray, 0, byteLength);
                    }
                }
            }
            return toString;
    ...
    static record Extractor(
        ...
        static class NameData {
            byte[] byteArray = new byte[NAME_BUFFER_SIZE];
            long[] longArray = new long[NAME_BUFFER_SIZE >> 4];
            int    byteLength;
            int    longLength;
            int    hash;
        }
        ...
        Statistic extract() throws IOException {
            ...
            while (buffer.hasRemaining() && (buffer.position() <= stopPos)) {
                ...
                var stationName = new StationName(nameData.byteArray, nameData.longArray, nameData.byteLength, nameData.longLength, nameData.hash);
        ...
        void readNameInto(NameData nameData) {
            var byteArray = nameData.byteArray;
            var longArray = nameData.longArray;
            int byteIndex = 0;
            int longIndex = 0;
            int nameHash  = 1;
            ...
            longArray[longIndex++] = longValue;
            
            nameData.byteLength = byteIndex;
            nameData.longLength = longIndex;
    ...
    static Runner createRunner(String[] args) {
        ...
        var executor = timed
                     ? (Runner)(CalculateAverage_NawaMan4::timedRun)
                     : (Runner)(CalculateAverage_NawaMan4::regularRun);
        return executor;
    }
